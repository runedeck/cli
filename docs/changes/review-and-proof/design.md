...
- `src/cli/proof/`: `Frontmatter { change, head, recorded, transcript, scenes }`, `Scene { scenario, kind, model }`, the README reader and writer, `scaffold`, `run`, `check`. The runner parses the fences itself (trycmd's `TestCases::run` returns nothing and keeps outcomes private) and executes each command through `snapbox::cmd::Command`, asserting `code` and `stdout_eq` with snapbox's `[..]` and `...` filters, so every scene has an outcome the runner owns. Each scene gets its own temporary directory, and a `$ cd` first line sets the working directory under the repository root.
