...
Block entries MUST record the channel a verdict arrived through: `cli` for `rune adopt verdict`, `tty` for the interactive review mode. The interactive mode (`rune adopt review`, own change) MUST require a controlling TTY and MUST refuse piped input.
