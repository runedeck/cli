...
`rune sign` MUST create an empty commit signed with the owner's OpenPGP key on the current branch and push it to the branch's remote, so the owner's key attests the exact state under review.
...
A seal MUST attest only the history beneath it: verification helpers MUST treat a branch whose latest commit is not the owner-verified seal as unsealed.
...
`rune sign --tag <name>` MUST create the owner-signed annotated tag on the named commit (HEAD by default), verify it locally, and push only that tag, covering release tags and post-merge seals with one ceremony.
...
`rune sign --verify [ref]` MUST report whether the ref's latest commit or named tag carries a valid signature matching a key in the repository's `KEYS` file, exiting nonzero when it does not.
