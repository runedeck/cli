...
`rune sign open <bookmark>` MUST refuse a protected branch (`main`, `master`, `trunk`) and a bookmark with no open
pull request at its head. The body MUST come from `docs/changes/<id>/pull-request.md` when the bookmark is
`change/<id>` and that file exists, else from `--body-file`. Before the key the command MUST print the bookmark,
the base, the diff stat against the base, and the body. Every jj call MUST pin `git.executable-path` to `git`.
...
### Requirement: The open-seal commit names the draft

After the print `rune sign open` MUST write an empty commit above the head whose message is `open-seal:`, one space,
and one JSON object with `repo`, `base`, `pull_request`, `tree`, and `nonce`, where `pull_request` is the draft's
number and `nonce` is thirty-two random bytes as lowercase hex, move the bookmark onto it, sign it through the
queue's pinned `jj sign`, and read the result back through git: sole parent the head, the head's tree, the message
as signed, a signature from a key in `KEYS` as read from the protected branch on `origin`.
...
### Requirement: The seal reaches the remote under pinned settings

`rune sign open` MUST then push the seal to `origin` with `git push` under a lease on the head that was sealed,
with every git setting that names a program pinned on the command line (`core.hooksPath` to an empty directory,
`core.sshCommand` to `ssh`, `core.askPass` and `core.gitProxy` empty, `credential.helper` reset to the system and
user values), so nothing in the repository's own tree or config runs as the owner.

#### Scenario: Seal pushed under a lease

- **WHEN** the repository's own tree configures a hook or a program and the seal is pushed
- **THEN** `origin` holds the sealed head under a lease on the head that was sealed
- **AND** no program the repository names runs as the owner

### Requirement: The draft carries the nonce

`rune sign open` MUST then flip the draft with `gh pr ready` and replace the body with the validated body plus one
line `Open-Seal-Nonce: <nonce>`. With `--queue` it MUST record a request of kind `open` and sign nothing.
`rune sign next` completes such a request after the acknowledgment.

#### Scenario: Draft readied with the nonce

- **WHEN** the seal is on `origin`
- **THEN** the draft is ready and its body is the validated body plus one `Open-Seal-Nonce:` line

#### Scenario: Session queues the open request

- **WHEN** a session runs `rune sign open change/x --queue`
- **THEN** the command records a request of kind `open` and signs nothing
- **AND** `rune sign next` completes that request after the acknowledgment
...
Every lane in `lanes` MUST be `completed`, `completed-no-findings`, `skipped`, `ineligible`, `failed`, or
`rate-limited`, every thread in `threads` is disposed `fixed` or `rejected`, every check from
`gh pr checks --required` is in the `pass` bucket, and the receipt qualifies the head. A thread disposed `owner` or
without a disposition MUST be refused by name. A head that carries the owner's open-seal MAY be submitted.
`rune sign queue <bookmark>` keeps the in-place signing of CLI-0041 and reads no ledger.
...
### Requirement: The verdict binds to the reviewed head and its generation

`rune sign submit` MUST refuse unless the ledger's `reviewed_sha` is the bookmark's head and either its `verdict`
binds to that `reviewed_sha` at the ledger's `generation` with `verdict` equal to `clean`, or `verdict` is null and
`coverage` starts with `free lanes only`, in which case the coverage is recorded as `free-lanes-only` with the
coverage text as the reason.
...
### Requirement: The ledger comes from the controller app
...
### Requirement: Submit checks the artifact and the pull request

The command MUST download the artifact through `gh api
...
`pull_request` MUST be its number.
...
### Requirement: The request records the coverage it was admitted on

The command MUST record the repository, pull request number, base, `reviewed_sha`, `generation`, `digest`, verdict,
reason, lanes, and threads on the request as its coverage.
...
### Requirement: The merge-seal names the reviewed head and its generation

On acknowledgment of a merge request the command MUST write an empty commit whose sole parent is
...
from the pull request's head, and create the bookmark `adopt/<number>` at that commit.

#### Scenario: Adopt refused without admin permission

- **WHEN** `gh api repos/<owner>/<name>` does not report `permissions.admin`
- **THEN** the command exits nonzero before it reads the pull request

### Requirement: The adopted head is sealed before anything is pushed

`rune sign adopt <number>` MUST print the view and sign the open-seal above the fetched head, with `pull_request`
naming the outside pull request, before anything is pushed. Only then it MUST push the sealed bookmark to `origin`
under a lease that the branch did not exist, wait for the app's draft on `adopt/<number>` through repeated
`gh pr list --head`, flip that draft, and write the nonce into its body. A refused key MUST leave nothing on
`origin` and the bookmark back on the fetched head.
...
The open-seal MUST be searched in `refs/remotes/origin/<base>..<reviewed>` for the pull request's
...
nonce as an `Open-Seal-Nonce:` line.
...
### Requirement: A copied nonce is reported and does not fail the check

Another open body that carries the nonce MUST be reported and MUST NOT fail the check.
...
### Requirement: The check binds one open pull request to the ref

The pull request under check is the one `--pull-request` names, which MUST be open, or else the one open pull
request whose head is the ref, read through `gh pr list --state open`. Two at one head fail the check. The pull
request's head MUST be the ref.
...
### Requirement: The verifier resolves the ref and KEYS through git
...
is an error. The command MUST print one line per check, MUST exit 0 when every check holds, 1 when one fails, and 2
...
### Requirement: A merge-seal sits on the reviewed head it names

When the ref's message is a merge-seal the command MUST check that the ref's sole parent is the `reviewed_sha` the
message names, that the ref's tree equals that parent's tree, and that the signature is from a `KEYS` key, and
MUST then search for the open-seal beneath `reviewed_sha`. Otherwise it MUST search beneath the ref.
