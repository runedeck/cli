...
Define the repository roots, the artifact dialect, and the validation behavior of `rune spec`. Conversion between the native root and an `openspec/` tree, and the recovery of interrupted mutations, are in `spec-interop`.
