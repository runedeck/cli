# Behavior proof: owner-signing-queue

`record.sh` holds one scene per scenario of the `owner-signing-queue` delta specification, in the grammar of
`docs/proofs/driver.sh`. `proof.cast` is its recording against the built binary on 2026-09-15, exit 0. `proof.txt` is the
...
```sh
RUNE=target/debug/rune asciinema rec --command "bash docs/proofs/owner-signing-queue/record.sh" \
...
```sh
RUNE=target/debug/rune python3 docs/proofs/cast.py docs/proofs/owner-signing-queue/record.sh proof.cast
```
