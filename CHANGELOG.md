...
- Feed a step's standard input from `< text` lines in its fence, recorded as command lines, so a scene carries its own script or brief.
...
- Add the built-in model route `kimi` (`kimi-k3`).
