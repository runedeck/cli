...
- `rune sign queue <bookmark>` refuses a head that `origin/<bookmark>` already holds or sits above, because signing in place would rewrite a published tip. `rune sign next` treats jj's immutable-commit refusal as a non-retried failure with the same reason, prints the pinentry focus hint on every timeout, and prints the `jj git push` line after a signed head without running it.
