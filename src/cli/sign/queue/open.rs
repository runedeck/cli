...
use super::repo::{Head, Repo, Signing};
...
/// Bookmarks that are never a session's own branch.
const PROTECTED: [&str; 3] = ["main", "master", "trunk"];
const PUSH_HOOK: &str = ".githooks/jj-push";
/// The remote the guarded push targets and the seal names.
...
pub(crate) fn open(
    ...
    let keys = crate::cli::sign::keys_fingerprints(&repo.workspace.join("KEYS"))?;
...
/// key, run at once or from the queue.
pub(crate) fn complete(
    ...
    // The seal is signed and on the bookmark. Anything that fails from here
    // is recorded so the owner finishes by hand, and never signed again.
    let finish = guarded_push(repo, &request.bookmark)
        .and_then(|()| gh::ready(&open.repo, open.pull_request))
        .and_then(|()| {
            gh::edit_body(
                &open.repo,
                open.pull_request,
                &seal::body_with_nonce(&open.body, &nonce),
            )
        });
    Ok(match finish {
        Ok(()) => Attempt::Signed(sealed),
        Err(error) => Attempt::Failed(format!(
            "open-seal {} is signed on {}: {error}",
            short(&sealed),
            request.bookmark
        )),
    })
...
/// The repository's checked push, run from the workspace with stdio
/// inherited so its checks and prompts reach the owner.
fn guarded_push(repo: &Repo, bookmark: &str) -> Result<(), Error> {
    let hook = repo.workspace.join(PUSH_HOOK);
    if !hook.is_file() {
        return Err(Error::new(
            ErrorKind::Config,
            format!(
                "no guarded push at {}: the open-seal is signed and unpushed",
                hook.display()
            ),
        ));
    }
    let status = std::process::Command::new("bash")
        ...
        .status()
        .map_err(|error| Error::new(ErrorKind::Io, format!("cannot run the push hook: {error}")))?;
    if status.success() {
        Ok(())
    } else {
        Err(Error::new(
            ErrorKind::Io,
            format!("the guarded push of {bookmark} failed: the open-seal is signed and unpushed"),
        ))
    }
}
...
/// `rune sign adopt <number>`: fetch the outside head, put it on
/// `adopt/<number>`, push, wait for the app's draft, and open it.
pub(crate) fn adopt(number: u64, repository: Option<&Path>, json: bool) -> Result<i32, Error> {
    ...
    guarded_push(&repo, &bookmark)?;
    let draft = wait_for_draft(&slug, &bookmark)?;
    let body = tempfile::Builder::new()
        .suffix(".md")
        .tempfile()
        .map_err(|error| Error::new(ErrorKind::Io, format!("cannot write the body: {error}")))?;
    std::fs::write(body.path(), &outside.body)
        .map_err(|error| Error::new(ErrorKind::Io, format!("cannot write the body: {error}")))?;
    println!(
        "adopted #{number} ({}) onto {bookmark}, draft #{}",
        short(&fetched),
        draft.number
    );
    open(
...
fn wait_for_draft(slug: &str, bookmark: &str) -> Result<PullRequest, Error> {
    ...
    Err(refusal(format!(
        "no draft opened for {bookmark}: run `rune sign open {bookmark} --body-file <FILE>` once it exists"
    )))
}
