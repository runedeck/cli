...
.PHONY: help build install validate test clean worktree worktree-done

help:
	@echo "  make build      compile the rune binary"
	@echo "  make install    install check tools, build, symlink, activate hooks"
...
worktree-done:
	...
	if [ -d .jj ]; then \
	    ...
	    git_directory=$$(git rev-parse --git-dir 2>/dev/null) \
	        || { echo "worktree-done needs a colocated jj repository (no git store found)."; exit 1; }; \
	    target_index=$$(mktemp); \
	    rm -f "$$target_index"; \
	    ...
	    if command -v trash >/dev/null 2>&1; then trash "$$path"; else rm -rf "$$path"; fi; \
